                    ...
                    <td>
                        <div class="d-flex gap-1">
                            <button class="btn btn-sm btn-outline-info btn-icon"
                                onclick="viewNote({{ $note->id }})" title="View">
                                <i class="bi bi-eye"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-primary btn-icon"
                                onclick="editNote({{ $note->id }},{{ json_encode($note) }})" title="Edit">
                                <i class="bi bi-pencil"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger btn-icon"
                                onclick="deleteNote({{ $note->id }},'{{ addslashes($note->title) }}')" title="Delete">
        ...
        <div class="modal-content border-0 shadow">
            <div class="modal-header border-0 pb-0">
                <h6 class="modal-title fw-bold">
                    <i class="bi bi-journal-plus text-primary me-2"></i>New Meeting Note
                </h6>
            ...
            <form method="POST" action="{{ route('notes.store') }}">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-sm-6">
                            <label class="form-label" style="font-size:.85rem">Category *</label>
                        ...
                        <div class="col-sm-6">
                            <label class="form-label" style="font-size:.85rem">Meeting Date *</label>
                            <input type="date" name="meeting_date" class="form-control" value="{{ date('Y-m-d') }}" required>
                        </div>
                        <div class="col-sm-6">
                            <label class="form-label" style="font-size:.85rem">Location</label>
                            <input type="text" name="location" class="form-control" placeholder="Room / Online">
                        </div>
                        <div class="col-sm-6">
                            <label class="form-label" style="font-size:.85rem">Attendees Count</label>
                            <input type="number" name="attendees_count" class="form-control" value="1" min="1">
                        </div>
                        <div class="col-12">
                            <label class="form-label" style="font-size:.85rem">Meeting Notes *</label>
                            <textarea name="content" class="form-control" rows="5"
                                placeholder="Write meeting notes, decisions, action items…" required></textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label" style="font-size:.85rem">Action Items</label>
                            <textarea name="action_items" class="form-control" rows="2"
                                placeholder="List of action items…"></textarea>
                        </div>
                    </div>
                </div>
...
<div class="modal fade" id="editNoteModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow">
            <div class="modal-header border-0 pb-0">
                <h6 class="modal-title fw-bold">
                    <i class="bi bi-pencil text-primary me-2"></i>Edit Meeting Note
                </h6>
                <button class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form method="POST" id="editNoteForm">
                ...
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check2 me-1"></i>Update Note
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body">
                <div class="d-flex gap-2 mb-3">
                    <span class="badge rounded-pill bg-primary" id="vCategory"></span>
                    <span class="text-muted" style="font-size:.8rem" id="vDate"></span>
                    <span class="text-muted" style="font-size:.8rem" id="vLocation"></span>
                </div>
                <div class="mb-3">
                    <div class="fw-bold mb-1" style="font-size:.8rem;text-transform:uppercase;color:#64748b">Notes</div>
                    <div id="vContent" style="font-size:.875rem;white-space:pre-wrap;background:#f8fafc;border-radius:8px;padding:1rem"></div>
                </div>
                <div>
                    <div class="fw-bold mb-1" style="font-size:.8rem;text-transform:uppercase;color:#64748b">Action Items</div>
                    <div id="vActionItems" style="font-size:.875rem;white-space:pre-wrap;background:#f8fafc;border-radius:8px;padding:1rem"></div>
                </div>
            </div>
        </div>
    </div>
...
function editNote(id, note) {
    ...
    document.getElementById('eCategory').value     = note.category;
    document.getElementById('eMeetingDate').value  = note.meeting_date ? note.meeting_date.split('T')[0] : '';
    document.getElementById('eLocation').value     = note.location || '';
    document.getElementById('eAttendees').value    = note.attendees_count || 1;
    document.getElementById('eContent').value      = note.content;
    document.getElementById('eActionItems').value  = note.action_items || '';
...
function viewNote(id) {
    fetch('/notes/' + id + '/json')
        .then(r => r.json())
        .then(note => {
            document.getElementById('vTitle').textContent       = note.title;
            document.getElementById('vCategory').textContent    = note.category;
            document.getElementById('vDate').textContent        = '📅 ' + note.meeting_date;
            document.getElementById('vLocation').textContent    = note.location ? '📍 ' + note.location : '';
            document.getElementById('vContent').textContent     = note.content;
            document.getElementById('vActionItems').textContent = note.action_items || 'None';
